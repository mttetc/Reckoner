...
async def search(
    ...
    hits = await search_knowledge(store, game.value, q, k=k, patch=patch)
    return [
...
async def patches(store: Knowledge, game: GameId) -> list[PatchInfo]:
    return [PatchInfo(**p) for p in await list_patches(store, game.value)]


@router.get("/stats", response_model=KnowledgeStats)
async def stats(store: Knowledge) -> KnowledgeStats:
    return KnowledgeStats(**await knowledge_stats(store))
