...
backend/            FastAPI · Python ≥ 3.12
  ...
  app/agent/        tool registry + evidence, LLM clients (Ollama/OpenAI-compatible, Anthropic, scripted), LangGraph state machine (graph.py) with conversation memory (memory.py: Postgres checkpoints, thread_id); hand-written loop kept behind RECKONER_AGENT_ENGINE=loop; number audit
...
.venv/bin/python scripts/ask.py "Which of these is the tankiest?" --thread <id>       # continue a conversation
...
| § 8 metric | Adding PoE 2 knowledge touched one 5-line file under `games/poe2/`. World of Warcraft Retail (SimulationCraft profiles, engine, talents, Blizzard notes) and Classic (WoWSims exports, engine, talent trees): ~1,600 lines in `games/wow*/`; outside them two `GameId` values, two `MetricKey` values (hps, dtps), two registry lines, six engine settings, one generic `talents` endpoint that delegates to the adapter, two lines in the ingest script, and in the UI the game names, the what-if options per game and one game-specific card section (the talent grid) |
