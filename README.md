...
| § 8 metric | Adding PoE 2 knowledge touched one 5-line file under `games/poe2/`. World of Warcraft Retail (SimulationCraft profiles, engine, talents) and Classic (WoWSims exports): ~900 lines in `games/wow*/`; outside them two `GameId` values, two `MetricKey` values (hps, dtps), two registry lines, two engine settings, one generic `talents` endpoint that delegates to the adapter, and in the UI the game names plus one game-specific card section (the talent grid) |
